...
let CustomViewFont:UIFont = UIFont.systemFont(ofSize: 17)
...
//        let messageHeight = message.nsString.height(with: CustomViewFont!, constrainedToWidth: CustomViewWidth - TextLabelMarger * 2)
        ...
        customView.frame = CGRect.init(x: 0, y: 0, width: 270, height: UIScreen.main.bounds.size.width)
...
class HUDTools: NSObject {
    ...
    private static let _sharedInstance = HUDTools()
    
    class func getSharedInstance() -> HUDTools {
...
//        imageView.backgroundColor = .red
    ...
    func showMessage(_ view:UIView, msg:String, autoHidder:Bool) -> MBProgressHUD {
        ...
        hud.label.text = msg;
        hud.bezelView.layer.frame = CGRect.init(x: 0, y: 0, width: SCREENWIDTH, height: UIScreen.main.bounds.size.width)
        if autoHidder {
